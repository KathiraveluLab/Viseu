...
import edu.emory.viseu.overlay.util.ViseuConstants;
...
import static edu.emory.viseu.overlay.csv.scanner_util.ScannerUtil.getDiffInMins;
    ...
    public void addPatient(String patientID, String iStart, String iEnd, double duration,
        ...
        if (examsHashMap.containsKey(patientID)) {
            ...
            if (status) {
                ...
                patientObj.setStudyDescription(temp + ViseuConstants.STUDY_DESC_SEPARATOR + studyDescription);
                examsHashMap.put(patientID, patientObj);

                String tempID = patientID + ViseuConstants.PATIENT_DIFFERENTIATOR;
                if (examsHashMap.get(tempID)!= null) {
                    ...
                    if (status2) {
                        ...
                        patientObj.setStudyDescription(descFrom1 + ViseuConstants.STUDY_DESC_SEPARATOR + descFrom2);
                        examsHashMap.put(patientID, patientObj);
                        examsHashMap.remove(tempID);
                        String nextID = tempID + ViseuConstants.PATIENT_DIFFERENTIATOR;
                        while (examsHashMap.containsKey(nextID)) {
                            ...
                            tempID = nextID;
                            nextID = tempID + ViseuConstants.PATIENT_DIFFERENTIATOR;
                        }
                    }
                }
            } else {
                addPatient(patientID + ViseuConstants.PATIENT_DIFFERENTIATOR, iStart, iEnd, duration,
